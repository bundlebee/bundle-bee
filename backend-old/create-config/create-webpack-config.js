  ...
  return new Promise((resolve, reject) => {
    getFiles(rootDir).then(files => {
      ...
      if (webpackConfig.exists)
        valueForWebpackConfigOutput = // dev hardcoding the answer until finish parseConfig
          parseConfigForOutput(webpackConfig.content) ||
          'Users/bren/Codesmith/week-5/reactscrumboard/src/index.js';
      const response = {
        webpackConfig,
        entryIsInRoot,
        indexHtmlPath,
    ...
    if (createNewConfig) {
      ...
      createAndSaveWebpackConfig(
        ...
        .then(webpackConfigSavePath => {
          ...
          cmd.get(
            /* --profile --json > ${saveDestinationForStats} */
            `webpack --config ${webpackConfigSavePath} `,
            (err, res) => {
              if (err) reject(err);
              console.log(res);
